...
sub new
{
    ...
    $self->{'discord'} = Mojo::Discord->new(
        ...
        'callbacks' => {    # Discord Gateway Dispatch Event Types
            ...
            #'GUILD_MEMBER_REMOVE'   => sub { $self->discord_on_user_remove(shift) },
            #'GUILD_MEMBER_ADD'      => sub { $self->discord_on_user_join(shift) },
        },
...
sub discord_on_user_remove {
    my $self = shift;

    use MessageRequest;
    use GetIDKey;

    my @WH_Search = split(m/\|/, GetIDKeyFromC("258278694487982080"));
    ...
    foreach my $key (keys %{$self->{'users'}}) {
        if(!defined($self->{'users'}->{$key}{'bot'})) {
            $firstkey = $self->{'users'}->{$key}{'username'}."#".$self->{'users'}->{$key}{'discriminator'} . " <\@".$self->{'users'}->{$key}{'id'}.">";
            last;
        }
    }
    MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "'.$firstkey.' was kicked from the server"}', "1", "");
    
    use File::Slurp;
    append_file("server_log.txt", $firstkey . " is no longer in the server!\n");
}

# sub discord_on_user_join {
#     my $self = shift;
...
#         }
#     }
#     MakeDiscordPostJson("/webhooks/$WH_ID/$WH_Key", '{"content" : "'.$firstkey.' has joined the server"}', "1", "");

#     use File::Slurp;
#     append_file("server_log.txt", (Dumper $self));
# }
